...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
        fg[0] += k_cte  * CppAD::pow(vars[cte_start + t] - ref_cte, 2);
        fg[0] += k_epsi * CppAD::pow(vars[epsi_start + t] - ref_epsi, 2);
    ...
    for (int t = 0; t < N - 1; t++) {
        fg[0] += k_steering * CppAD::pow(vars[delta_start + t], 2); //50 to 500
        fg[0] += k_throttle * CppAD::pow(vars[a_start + t], 2);
    }
    ...
    for (int t = 0; t < N - 2; t++) {
        fg[0] += k_dsteering * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
        fg[0] += k_dthrottle * CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }
      //
      // Setup Constraints
      //
